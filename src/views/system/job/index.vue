<template>
  <div class="app-container">
    ...
    <pagination />
    <!--表单渲染-->
    <eForm :job-status="dict.boolean_status" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    changeEnabled(data, val) {
      this.$confirm('此操作将 "' + this.dict.label.boolean_status[val] + '" ' + data.name + '岗位, 是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        crud.crudMethod.edit(data).then(() => {
          crud.notify(this.dict.label.boolean_status[val] + '成功', 'success')
        }).catch(err => {
