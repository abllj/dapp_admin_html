<template>
  <div class="app-container">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0"
      ...
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item label="状态" prop="enabled">
          <el-radio v-for="item in dict.boolean_status" :key="item.id" v-model="form.enabled" :label="item.value">{{ item.label }}</el-radio>
        </el-form-item>
    ...
    <el-table ref="table" v-loading="crud.loading" :tree-props="{children: 'children', hasChildren: 'hasChildren'}"
      :data="crud.data" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
  ...
  export default {
    ...
    methods: {
      ...
      changeEnabled(data, val) {
        this.$confirm('此操作将 "' + this.dict.label.boolean_status[val] + '" ' + data.name + '部门, 是否继续？', '提示', {
