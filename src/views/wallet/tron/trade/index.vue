      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;"
        ...
        <el-table-column align="center" prop="txId" label="交易ID" show-overflow-tooltip>
          <template slot-scope="scope">
            <a :href="'https://tronscan.io/#/transaction/'+scope.row.txId" target="_blank" style="color: #0066CC;">{{scope.row.txId}}</a>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="fromAddr" label="转入地址" show-overflow-tooltip>
          <template slot-scope="scope">
            <a :href="'https://tronscan.io/#/address/'+scope.row.fromAddr" target="_blank" style="color: #0066CC;">{{scope.row.fromAddr}}</a>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="toAddr" label="接受地址" show-overflow-tooltip>
          <template slot-scope="scope">
            <a :href="'https://tronscan.io/#/address/'+scope.row.toAddr" target="_blank" style="color: #0066CC;">{{scope.row.toAddr}}</a>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="collectId" label="归集ID" show-overflow-tooltip>
          <template slot-scope="scope">
            <a :href="'https://tronscan.io/#/transaction/'+scope.row.collectId" target="_blank" style="color: #0066CC;">{{scope.row.collectId}}</a>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" width="135px" prop="gmtCreate" label="创建时间">
          <template slot-scope="scope">
            <span>{{ parseTime(scope.row.gmtCreate) }}</span>
          </template>
        </el-table-column>
        <el-table-column v-permission="['admin','walletTrxTrade:edit']" label="操作" width="70px"
          align="center">
          <template slot-scope="scope">
...
<script>
  import crudTrxTrade from '@/api/wallet/tron/trade/walletTrxTrade'
  import CRUD, {
    crud,
    form,
    header,
  ...
  export default {
    ...
    data() {
      return {
        permission: {
          add: ['walletTrxTrade:add'],
          edit: ['admin', 'walletTrxTrade:edit'],
          del: [ 'walletTrxTrade:del']
        },
        rules: {
          status: [{
            required: true,
